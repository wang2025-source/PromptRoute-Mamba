...
try:
    ...
    raise ImportError(
        "PromptRoute-Mamba requires mamba-ssm and causal-conv1d. "
        "Install the dependencies from environment.yaml or requirements.txt."
    ) from exc

# =============================================================================
# 前沿物理算子区 (升级军工级 FP32 防崩护盾)
# =============================================================================
class LocalSpatialVariance(nn.Module):
    ...
    def forward(self, x):
        x_f = x.float()
        # 【性能提升外挂】：使用 reflect 填充替代 zero 填充，消除融合图像四周的虚假黑边伪影
...
# =============================================================================
# [CVPR 2024 级外挂 1] Star-Gated Refinement (SGR)
# =============================================================================
...
# =============================================================================
# [顶会级外挂 2] 拉普拉斯物理金字塔边缘增强器 (LPEB)
# =============================================================================
class LaplacianPyramidEdgeBooster(nn.Module):
    def __init__(self, dim):
        super().__init__()
        # 【性能提升外挂】：将 4 邻域升级为 8 邻域拉普拉斯核，全面捕获对角线高频细节
...
# =============================================================================
# [全新提分外挂 3] 对比互补注意力机制 (Contrastive Complementary Attention, CCA)
# =============================================================================
...
# =============================================================================
# 核心架构区：原有多尺度模块 (升级融合 Star-Gated Refinement)
# =============================================================================
...
class MaxInformationChannelAmplifier(nn.Module):
    ...
    def forward(self, fused, v, i):
        # 【防崩修复 2】: 弃用原生 torch.var，采用带底衬 1e-6 的安全计算，防止纯黑图像方差为0
...
class CrossSequenceMambaFusion(nn.Module):
    ...
    def forward(self, v, i):
            ...
            # 【性能提升外挂】：采用交错拼接(Interleave)，打破 Mamba 的单向因果限制
            # 原本是 VVVVIII，现在变成 VIVIVIVI，让模态在像素级充分交互
...
# =============================================================================
# 【修改 4】将聚类模块移动到 AdvancedBaseFusion 之前，避免加载时找不到类的错误
# =============================================================================
class DeepUnfoldedSemanticClustering(nn.Module):
    def __init__(self, dim, num_clusters=8, temperature=0.1):
        ...
        # [全新外挂] 物理先验 Prompt 映射器
        # 接收 2 通道先验 (可见光纹理 + 红外强度)，输出路由偏置
    ...
    def forward(self, v, i):
        ...
        # ==========================================================
        # 1. 物理先验提取 (Physical Prior Extraction)
        # ==========================================================
        # 可见光先验：局部方差（代表纹理丰富度）
        ...
        v_prior = v_prior / (torch.amax(v_prior, dim=[2, 3], keepdim=True) + 1e-6) # 归一化

        # 红外先验：局部均值（代表热辐射强度）
        i_prior = F.avg_pool2d(i_f, 3, stride=1, padding=1)
        i_prior = i_prior / (torch.amax(i_prior, dim=[2, 3], keepdim=True) + 1e-6) # 归一化
        ...
        # ==========================================================
        # 2. 先验引导的专家路由 (Prior-Guided Expert Routing)
        # ==========================================================
        # 将先验 Prompt 注入特征，再进行扁平化和相似度计算
...
class AdvancedBaseFusion(nn.Module):
    ...
    # 【终极防崩护盾】：强制整个基础融合链路在 FP32 下运行，无视外层 AMP，彻底杜绝 Mamba 状态溢出！
    @torch.amp.autocast('cuda', enabled=False)
    def forward(self, v, i):
        # 强制将输入特征转换为单精度，冲破 FP16 65504 的天花板
...
class SpatialChannelDetailFusion(nn.Module):
    ...
    # 【终极防崩护盾】：强制细节链路在 FP32 下运行，杜绝 Laplacian 金字塔的 8 倍放大导致 FP16 inf 溢出！
    @torch.amp.autocast('cuda', enabled=False)
    def forward(self, v, i):
        # 强制特征升维至 FP32
        v_f, i_f = v.float(), i.float()
...
class Mamba_Decoder(nn.Module):
    ...
    # 【防崩修复 3】: 为解码器加挂 FP32 护盾，防止输入的高精度特征强制降维至 FP16 溢出
    @torch.amp.autocast('cuda', enabled=False)
